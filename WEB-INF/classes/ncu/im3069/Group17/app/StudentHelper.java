...
public class StudentHelper {
	...
	public JSONObject getNameByID(String id) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令，?為變數的意思 */
			String sql = "SELECT `name` FROM `sa_sharesport`.`students` WHERE `id` = ? LIMIT 1";
	...
	/**
	 * 透過cookie email, password checkLogin學生資料
	 *
	 * @param email    學生編號
	 * @param password 學生密碼
	 * @return the JSON object 回傳SQL執行結果與該學生編號之學生資料
	...
	public JSONObject getLogin(String email, String password) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT * FROM `sa_sharesport`.`students` WHERE `email` = ?  and `password` = ? and `status` = ? LIMIT 1";
	...
	public int checkLogin(String email, String password) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT count(*) FROM `sa_sharesport`.`students` WHERE `email` = ?  and `password` = ? and `status` = ? LIMIT 1";
	 ...
	 * 上傳學生之照片
	 *
	 * @param s 一名學生之Coach物件
	 * @return the JSONObject 回傳SQL指令執行結果與執行之資料
	 */
	public JSONObject saveImage(Student s) {
		/** 紀錄回傳之資料 */
		JSONArray jsa = new JSONArray();
		/** 記錄實際執行之SQL指令 */
		String exexcute_sql = "";
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "Update `sa_sharesport`.`students` SET `image` = ? WHERE `id` = ?";
			/** 取得所需之參數 */

			String image = s.getName(); // 因為用同一個建構子取得 filePath 變成getName方法
			int id = s.getID();
