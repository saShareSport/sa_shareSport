...
//        response.put("time", duration);
	...
	public JSONObject getLogin(String email, String password) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT * FROM `sa_sharesport`.`coaches` WHERE `email` = ?  and `password` = ? and `status` = ? LIMIT 1";
	...
	public int checkLogin(String email, String password) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT count(*) FROM `sa_sharesport`.`coaches` WHERE `email` = ?  and `password` = ? and `status` = ? LIMIT 1";
